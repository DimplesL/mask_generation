...
def fpn_classifier_graph(rois, feature_maps,
    ...
    # get the class and box: 80(class number)
...
def get_c5_feature(features_c5_map):
    features = features_c5_map[0]
    return features
...
def build_fpn_mask_graph(rois, feature_maps,
    ...
    # get the mask: 28*28*80(class number)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            features_c5_get = get_c5_feature(features_c5_map)

            # image_batch_size = np.shape(input_image)[0]
            # print(np.shape(input_image)[0])
            # Detections
            # output is [batch, num_detections, (y1, x1, y2, x2, class_id, score)] in image coordinates
            detections = DetectionLayer(config, name="mrcnn_detection")(
                [rpn_rois, mrcnn_class, mrcnn_bbox, input_image_meta]) #[image_batch_size,
            ...
            model = KM.Model([input_image, input_image_meta],
                             [detections, mrcnn_class, mrcnn_bbox,
                                 mrcnn_mask, rpn_rois, rpn_class, rpn_bbox, features_c5_get],
                             name='mask_rcnn')
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        self.keras_model.fit_generator(
            ...
            callbacks=callbacks,
            validation_data= val_generator,#next(val_generator),
    ...
    def detect(self, images, verbose=0):
        ...
        detections, mrcnn_class, mrcnn_bbox, mrcnn_mask, \
            rois, rpn_class, rpn_bbox, features_c5 =\
        ...
        for i, image in enumerate(images):
            ...
            features_res = features_c5[i]
            #origin_masks = mrcnn_mask[i]
            #print(np.shape(origin_masks),np.shape(mrcnn_mask[i]))
            results.append({
                "rois": final_rois,
        ...
        #def generator_1(molded_images, image_metas):
          #  return [molded_images, image_metas]
        # detections, mrcnn_class, mrcnn_bbox, mrcnn_mask, \
        # rois, rpn_class, rpn_bbox, features_c5 =\
        #     self.keras_model.predict_generator(generator_1, steps=None, max_queue_size=10, workers=1, use_multiprocessing=True, verbose=0)

        # detections, mrcnn_class, mrcnn_bbox, mrcnn_mask, \
        #     rois, rpn_class, rpn_bbox, features_c5 =\
        #     self.keras_model.predict_on_batch([molded_images, image_metas])
        # m = []
        # for i, image in enumerate(images):
        #     final_rois, final_class_ids, final_scores, final_masks=\
        #         self.unmold_detections(detections[i], mrcnn_mask[i],
        ...
        #     features_res = features_c5[i]
        #     origin_masks = mrcnn_mask[i]
        #     # print(np.shape(origin_masks),np.shape(mrcnn_mask))
        #     m.append({
        #         "rois": final_rois,
        #         "class_ids": final_class_ids,
        #         "scores": final_scores,
